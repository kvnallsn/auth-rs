...
impl Hasher {
    ...
    pub fn hash<S: AsRef<str>>(&self, password: S) -> Result<String, Error> {
        match self {
            Hasher::Argon2(cfg) => {
                // use a 16-byte salt
                let mut salt = [0u8; 16];
                ...
                argon2::hash_encoded(password.as_ref().as_bytes(), &salt, cfg)
            }
        }
    }

    pub fn verify<S, H>(&self, password: S, hash: H) -> Result<bool, Error>
    where
        S: AsRef<str>,
        H: AsRef<str>,
    {
